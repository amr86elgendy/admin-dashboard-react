...
const Radio = ({ label, name, ...rest }) => {
  return (
    <div className='flex items-center col-span-6 lg:col-span-6'>
      <label
        htmlFor={name}
        className={`mb-2 font-medium capitalize text-[#344357] ${name === 'featured' ? 'mr-12' : 'mr-4'}`}
      >
        {name}
      </label>
      <Field name={name}>
          ...
          return (
            <div className='relative'>
              <input
                ...
                checked={field.value}
              />
              <FaCheck color='white' className='absolute top-1.5 right-1.5 hidden peer-checked:block' size={10} />
            </div>
          );
        }}
      </Field>
